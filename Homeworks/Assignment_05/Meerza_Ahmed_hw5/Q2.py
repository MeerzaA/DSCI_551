...
def query_b(film_df):
    ...
    """
    # Implement the query here
    film_df = film_df.filter(film_df.length >= 60).groupBy('rating').agg(fc.avg('replacement_cost').alias('average'), fc.count('*').alias('cnt')).filter('cnt>=160').select('rating', 'average')
...
def query_d(actor_df, film_actor_df):
    ...
    return actor_df.join(film_actor_df, actor_df.actor_id == film_actor_df.actor_id).filter(film_actor_df.film_id.isin([1,2,3])).select('first_name', 'last_name').distinct().orderBy('first_name').limit(5)
...
def query_e(film_df):
    ...
    """
    # Implement the query here
    film_df = film_df.groupBy('rental_duration', 'rating').agg(fc.min('length').alias('Min'), fc.max('length').alias('Max'), fc.avg('length').alias('Average'), fc.count('length').alias('Count')).orderBy(
